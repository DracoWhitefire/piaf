...
fn samsung_sam_identification() {
    ...
    assert_eq!(caps.manufacture_date, Some(ManufactureDate::Manufactured { week: Some(1), year: 2018 }));
    assert_eq!(caps.edid_version, Some(EdidVersion { version: 1, revision: 3 }));
    assert_eq!(caps.digital, true);
    assert_eq!(caps.screen_size, Some(ScreenSize::Physical { width_cm: 111, height_cm: 62 }));
    assert_eq!(caps.preferred_image_size_mm, Some((1872, 1053)));
...
fn samsung_sam_supported_modes() {
    ...
    assert!(!caps.supported_modes.is_empty());
    assert!(caps.supported_modes.iter().any(|m| m.width == 4096 && m.height == 2160 && m.refresh_rate == 60));
    assert!(caps.supported_modes.iter().any(|m| m.width == 3840 && m.height == 2160 && m.refresh_rate == 60));
